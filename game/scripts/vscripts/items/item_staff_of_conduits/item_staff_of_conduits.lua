...
function modifier_item_staff_of_conduits:OnAttack(event)
    ...
    if caster:GetPrimaryAttribute() == DOTA_ATTRIBUTE_INTELLECT then
        bonusFromStats = (self.caster:GetIntellect()*(self.ability:GetSpecialValueFor("int_damage")/100))
    ...
    elseif caster:GetPrimaryAttribute() == DOTA_ATTRIBUTE_STRENGTH then
        ...
        -- They're universal
        bonusFromStats = (self.caster:GetIntellect()*(self.ability:GetSpecialValueFor("int_damage")/100)) + (self.caster:GetAgility()*(self.ability:GetSpecialValueFor("agi_damage")/100)) + (self.caster:GetStrength()*(self.ability:GetSpecialValueFor("str_damage")/100)) + (self.caster:GetMaxHealth() * (self.ability:GetSpecialValueFor("str_damage_from_hp_pct")/100))
    end
