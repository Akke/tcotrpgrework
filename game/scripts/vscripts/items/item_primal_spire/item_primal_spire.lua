LinkLuaModifier("modifier_item_primal_spire", "items/item_primal_spire/item_primal_spire.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_item_primal_spire:OnCreated()
    ...
    self.intellect = self:GetParent():GetIntellect()
...
function modifier_item_primal_spire:OnIntervalThink()
    ...
    if self.attribute == DOTA_ATTRIBUTE_INTELLECT then
        if self:GetParent():GetIntellect() > self.intellect then
            self.intellect = self:GetParent():GetIntellect()
            self:InvokeAttribute()
        end
    else
