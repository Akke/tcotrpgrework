LinkLuaModifier("hero_akasha_scream_of_pain_modifier", "heroes/hero_akasha/scream_of_pain.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("hero_akasha_scream_of_pain_modifier_debuff", "heroes/hero_akasha/scream_of_pain.lua", LUA_MODIFIER_MOTION_NONE)
...
function hero_akasha_scream_of_pain:OnProjectileHit(hTarget, vLocation)
    local caster = self:GetCaster()
    local damage = self:GetLevelSpecialValueFor("damage", (self:GetLevel() - 1)) + (caster:GetIntellect()*(self:GetSpecialValueFor("int_to_damage")/100))
