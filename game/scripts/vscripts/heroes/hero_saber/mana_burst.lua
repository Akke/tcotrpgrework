...
modifier_saber_mana_burst = class({
	...
	DeclareFunctions = function() return {MODIFIER_PROPERTY_PHYSICAL_ARMOR_TOTAL_PERCENTAGE} end,
...
function modifier_saber_mana_burst:GetModifierPhysicalArmorTotal_Percentage()
...
if IsServer() then
	...
	function modifier_saber_mana_burst:OnIntervalThink()
		...
		if parent:IsAlive() then
			...
			if ability:GetAutoCastState() and not parent:IsChanneling() and manacost * 2 < parent:GetMana() then
