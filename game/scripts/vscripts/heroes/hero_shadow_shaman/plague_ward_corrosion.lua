LinkLuaModifier("modifier_plague_ward_corrosion", "heroes/hero_shadow_shaman/plague_ward_corrosion.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_plague_ward_corrosion_debuff", "heroes/hero_shadow_shaman/plague_ward_corrosion.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_plague_ward_corrosion_debuff:OnIntervalThink()
    local damage = (self:GetAbility():GetSpecialValueFor("damage") + (self:GetCaster():GetIntellect() * (self:GetAbility():GetSpecialValueFor("int_to_damage")/100))) * self:GetStackCount()
