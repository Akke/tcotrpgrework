modifier_carl_sun_strike_thinker = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_carl_sun_strike_thinker:IsHidden()
	return true
end

function modifier_carl_sun_strike_thinker:IsPurgable()
	return false
end

--------------------------------------------------------------------------------
-- Initializations
function modifier_carl_sun_strike_thinker:OnCreated( kv )
	if IsServer() then
		-- references
		self.damage = self:GetAbility():GetOrbSpecialValueFor("damage","e") + (self:GetCaster():GetIntellect() * (self:GetAbility():GetSpecialValueFor( "int_to_damage" )/100))
...
function modifier_carl_sun_strike_thinker:OnDestroy( kv )
	if IsServer() then
		-- Damage enemies
		local damageTable = {
			-- victim = target,
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_carl_sun_strike_thinker:PlayEffects1()
	-- Get Resources
...
function modifier_carl_sun_strike_thinker:PlayEffects2()
	-- Get Resources
