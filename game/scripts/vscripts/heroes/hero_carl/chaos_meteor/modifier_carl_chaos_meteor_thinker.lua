modifier_carl_chaos_meteor_thinker = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_carl_chaos_meteor_thinker:IsHidden()
	return true
end

--------------------------------------------------------------------------------
-- Initializations
function modifier_carl_chaos_meteor_thinker:OnCreated( kv )
	if IsServer() then
		-- references
		...
		self.radius = self:GetAbility():GetOrbSpecialValueFor( "area_of_effect", "w" )
		...
		self.initialStacks = self:GetAbility():GetOrbSpecialValueFor( "burn_initial_stacks", "q" )
		
		self.interval = 0.5
		self.duration = self:GetAbility():GetOrbSpecialValueFor( "burn_duration", "q" )
		...
		self.damageTable = {
			...
			damage = damage + (self:GetCaster():GetIntellect() * (self:GetAbility():GetSpecialValueFor( "int_to_damage" )/100)),
...
function modifier_carl_chaos_meteor_thinker:OnRefresh( kv )
	
end

function modifier_carl_chaos_meteor_thinker:OnDestroy( kv )
	if IsServer() then
		-- add vision
...
--------------------------------------------------------------------------------
-- Interval Effects
function modifier_carl_chaos_meteor_thinker:OnIntervalThink()
...
function modifier_carl_chaos_meteor_thinker:Burn()
	-- find enemies
	...
	for _,enemy in pairs(enemies) do
		...
		-- add modifier
		local burnMod = enemy:FindModifierByName("modifier_carl_chaos_meteor_burn")
		if burnmod == nil then
			local newBurnMod = enemy:AddNewModifier(
				self:GetCaster(), -- player source
				self:GetAbility(), -- ability source
				"modifier_carl_chaos_meteor_burn", -- modifier name
				{ duration = self.duration } -- kv
			)

			if newBurnMod ~= nil then
				newBurnMod:SetStackCount(self.initialStacks)
			end
		end
	end
end
...
--------------------------------------------------------------------------------
-- Motion effects
function modifier_carl_chaos_meteor_thinker:Move_Burn()
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_carl_chaos_meteor_thinker:PlayEffects1()
	-- Get Resources
...
function modifier_carl_chaos_meteor_thinker:PlayEffects2()
	-- Get Resources
