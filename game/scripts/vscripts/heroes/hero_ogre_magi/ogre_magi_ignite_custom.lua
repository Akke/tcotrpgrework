...
LinkLuaModifier( "modifier_ogre_magi_ignite_custom", "heroes/hero_ogre_magi/ogre_magi_ignite_custom.lua", LUA_MODIFIER_MOTION_NONE )
...
function modifier_ogre_magi_ignite_custom:OnRefresh( kv )
    -- references
    self.slow = self:GetAbility():GetSpecialValueFor( "slow_movement_speed_pct" )
    local damage = self:GetAbility():GetSpecialValueFor( "burn_damage" ) + (self:GetCaster():GetIntellect()*(self:GetAbility():GetSpecialValueFor("int_to_damage")/100))
...
function modifier_ogre_magi_ignite_custom:OnTooltip()
    return (self:GetAbility():GetSpecialValueFor("burn_damage")+((self:GetCaster():GetIntellect()*(self:GetAbility():GetSpecialValueFor("int_to_damage")/100))))*self:GetStackCount()
end
