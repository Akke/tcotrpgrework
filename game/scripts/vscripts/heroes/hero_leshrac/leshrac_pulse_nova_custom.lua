...
--------------------------------------------------------------------------------
-- Ability Start
function leshrac_pulse_nova_custom:GetManaCost(level)
    return self:GetCaster():GetMaxMana() * (self:GetSpecialValueFor("mana_cost_per_second")/100)
end
...
function modifier_leshrac_pulse_nova_custom:OnCreated( kv )
    ...
    local interval = 1 / self:GetParent():GetAttacksPerSecond()

    if interval > intervalMin then
...
function modifier_leshrac_pulse_nova_custom:Burn()
    ...
    for _,enemy in pairs(enemies) do
        ...
        if self.parent:HasScepter() then
            ...
            if lightningStorm ~= nil and lightningStorm:GetLevel() > 0 and lightningStorm:GetManaCost(-1) <= self.parent:GetMana() and lightningStorm:IsCooldownReady() then
                SpellCaster:Cast(lightningStorm, enemy, true)
            end
        end
    end
end
