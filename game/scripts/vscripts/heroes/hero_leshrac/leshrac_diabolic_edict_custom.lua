...
function modifier_leshrac_diabolic_edict_custom:OnTakeDamage(event)
    ...
    if event.inflictor ~= nil then
        if event.inflictor == self:GetAbility() or string.find(event.inflictor:GetAbilityName(), "staff_of_dragon") then return end
    end
...
function modifier_leshrac_diabolic_edict_custom_debuff:OnIntervalThink()
    ...
    ApplyDamage({
        ...
        victim = parent,
        damage = (self:GetAbility():GetSpecialValueFor("damage") + (self:GetCaster():GetIntellect() * (self:GetAbility():GetSpecialValueFor("int_to_damage")/100))) * self:GetStackCount(),
...
function modifier_leshrac_diabolic_edict_custom_debuff:OnTakeDamage(event)
    if not IsServer() then return end
    ...
    local restore = event.damage * (ability:GetSpecialValueFor("mana_leech")/100)

    caster:GiveMana(restore)

    SendOverheadEventMessage(nil, OVERHEAD_ALERT_MANA_ADD, caster, restore, nil)
end