...
LinkLuaModifier( "modifier_dazzle_poison_touch_custom", "heroes/hero_dazzle/dazzle_poison_touch_custom.lua", LUA_MODIFIER_MOTION_NONE )
...
function modifier_dazzle_poison_touch_custom:OnIntervalThink()
    -- apply damage
    local damage = ((self:GetCaster():GetIntellect() * (self:GetAbility():GetSpecialValueFor("int_to_damage")/100)) + self:GetAbility():GetSpecialValueFor( "damage" )) * self:GetStackCount()
