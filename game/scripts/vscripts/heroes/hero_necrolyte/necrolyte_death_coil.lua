LinkLuaModifier("modifier_necrolyte_death_coil", "heroes/hero_necrolyte/necrolyte_death_coil.lua", LUA_MODIFIER_MOTION_NONE)
...
necrolyte_death_coil = class(ItemBaseClass)
modifier_necrolyte_death_coil = class(necrolyte_death_coil)
-------------
function necrolyte_death_coil:GetIntrinsicModifierName()
    return "modifier_necrolyte_death_coil"
end

function necrolyte_death_coil:GetAOERadius()
    return self:GetSpecialValueFor("radius")
end

function necrolyte_death_coil:GetBehavior()
...
function necrolyte_death_coil:OnSpellStart()
    ...
    for _,victim in ipairs(victims) do
        if victim:IsAlive() and not victim:IsMagicImmune() and not victim:IsInvulnerable() then
            local effectName = "particles/units/heroes/hero_necrolyte/necrolyte_pulse_enemy.vpcf"
...
function necrolyte_death_coil:OnProjectileHit_ExtraData(target, location, extraData)
    local ability = self
    local caster = self:GetCaster()
    ...
    if extraData.enemy == 1 then
        ApplyDamage({
            victim = target,
            attacker = caster,
            damage = ability:GetSpecialValueFor("damage") + (caster:GetIntellect() * (ability:GetSpecialValueFor("int_to_damage")/100)),
...
function modifier_necrolyte_death_coil:OnCreated()
    if not IsServer() then return end

    self:StartIntervalThink(0.1)
end

function modifier_necrolyte_death_coil:OnIntervalThink()
