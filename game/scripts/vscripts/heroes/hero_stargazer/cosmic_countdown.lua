function CheckAbility(keys)
	...
	if ability:IsCooldownReady() and IsServer() then
        ...
        if preTotalInt < maxLimit and caster:GetIntellect() < maxLimit then 
          caster:ModifyIntellect(bonusInt)
        end

        if (preTotalStr >= maxLimit and preTotalAgi >= maxLimit and preTotalInt >= maxLimit) or (caster:GetStrength() >= maxLimit and caster:GetAgility() >= maxLimit and caster:GetIntellect() >= maxLimit) then 
