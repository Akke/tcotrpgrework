    ...
    if RollPercentage(ability:GetSpecialValueFor("chance")) and ability:IsCooldownReady() then
		...
		for _, enemy in ipairs(enemies) do
			ApplyDamage({
				victim = enemy,
				attacker = caster,
				damage = (ability:GetLevelSpecialValueFor("base_damage", level) + caster:GetIntellect()) + (caster:GetIntellect() * (ability:GetLevelSpecialValueFor("int_to_dmg_pct", level) * 0.01)),
