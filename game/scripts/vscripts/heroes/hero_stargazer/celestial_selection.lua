...
function modifier_stargazer_celestial_selection:OnDeath(event)
    ...
    if buffInt ~= nil then
        local preTotal = unit:GetIntellect() + (buffInt:GetStackCount() * ability:GetSpecialValueFor("int_gain"))
        if preTotal >= 1000000 or unit:GetIntellect() >= 1000000 then return false end
    end
    --
