...
function modifier_nevermore_ultimate_raze:OnAttackLanded(event)
    ...
    if not RollPseudoRandomPercentage(ability:GetSpecialValueFor("chance"), 99, unit) then return end

    if IsBossTCOTRPG(victim) then
        if (unit:GetAbsOrigin() - victim:GetAbsOrigin()):Length2D() > 900 then
            return
        end
    end
