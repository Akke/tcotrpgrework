...
function modifier_nevermore_presence_of_the_dark_lord_custom:GetAuraEntityReject( hEntity )
    return hEntity:GetTeamNumber() == self:GetCaster():GetTeamNumber()
end
--------------------------------------------------------------------------------
...
function modifier_nevermore_presence_of_the_dark_lord_custom:OnCreated( kv )
    ...
    self.armor_reduction = (self:GetCaster():GetStrength()+self:GetCaster():GetAgility()+self:GetCaster():GetIntellect()) * (self:GetAbility():GetSpecialValueFor( "agility_armor_reduction_pct" )/100)
