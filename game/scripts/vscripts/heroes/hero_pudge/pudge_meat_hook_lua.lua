...
if IsServer() then
	...
	function pudge_meat_hook_lua:OnSpellStart()
		...
		if bloodMod ~= nil then
			if bloodMod:GetStackCount() > 0 then
				hook_damage = hook_damage + (bloodMod:GetStackCount()*self:GetSpecialValueFor("hook_damage_per_stack"))
			end
		end
		...
		caster:AddNewModifier(caster, self, "modifier_meat_hook_followthrough_lua", {duration = hook_distance / hook_speed * hook_followthrough_constant}) --this line used to require scepter. but why?
		...
		for i = 0, hookCount-1 do
			...
			local hook_chain_pfx = ParticleManager:CreateParticle("particles/units/heroes/hero_pudge/pudge_meathook.vpcf", PATTACH_CUSTOMORIGIN, caster)
			...
			ParticleManager:SetParticleControl( hook_chain_pfx, 1, (self:GetCaster():GetOrigin() + base_direction * hook_distance) )
			ParticleManager:SetParticleControl( hook_chain_pfx, 2, Vector( hook_speed, hook_distance, hook_width ) )
			ParticleManager:SetParticleControl( hook_chain_pfx, 3, Vector( (hook_distance / hook_speed * 2), 0, 0 ) )
			ParticleManager:SetParticleControl( hook_chain_pfx, 4, Vector( 1, 0, 0 ) )
			ParticleManager:SetParticleControl( hook_chain_pfx, 5, Vector( 0, 0, 0 ) )
			ParticleManager:SetParticleControlEnt(
				hook_chain_pfx,
				7,
				self:GetCaster(),
			...
			local projbase = {
				...
				UnitTest = function(proj, unit)
					return UnitFilter(unit, DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, caster:GetTeamNumber()) == UF_SUCCESS
				end,
						...
						if pos then
							...
							Timers:CreateTimer(0.03, function()
								...
								newProjectile.Thinker = Timers:CreateTimer(function()
									if IsValidEntity(self) and IsValidEntity(caster) then
										ParticleManager:SetParticleControl(hook_chain_pfx, 1, newProjectile:GetPosition() + vHookOffset)
			...
			projectile.Thinker = Timers:CreateTimer(function()
				ParticleManager:SetParticleControl(hook_chain_pfx, 1, projectile:GetPosition() + vHookOffset)
				return 0.03
			end)
			projectile.hVictim = {}
	...
	function pudge_meat_hook_lua:OnProjectileHit(hTarget, vLocation, proj, hook_damage)
		...
		--if IsBossTCOTRPG(hTarget) then return false end
		if not IsCreepTCOTRPG(hTarget) and (hTarget:IsRealHero() and hTarget:GetTeamNumber() == caster:GetTeamNumber()) then return false end
		...
		if not proj.hVictim or not table.includes(proj.hVictim, hTarget) then
			...
			if hTarget:GetTeamNumber() ~= caster:GetTeamNumber() then
				...
				if not hTarget:IsAlive() and (hTarget:IsRealHero() or IsCreepTCOTRPG(hTarget)) then
...
if IsServer() then
	...
	function modifier_meat_hook_lua:OnIntervalThink(kv)
		local caster = self:GetCaster()
		local parent = self:GetParent()

		if (caster:GetAbsOrigin() - parent:GetAbsOrigin()):Length2D() > self:GetAbility():GetEffectiveCastRange(caster:GetAbsOrigin(), nil) then
			self:Destroy()
		end
	end
