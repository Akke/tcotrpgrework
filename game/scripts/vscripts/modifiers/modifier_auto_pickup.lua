...
function modifier_auto_pickup:OnCreated(params)
    ...
    local pid = PlayerResource:GetPlayer(self.player:GetPlayerID())
...
function modifier_auto_pickup:OnIntervalThink()
    ...
    if not self:GetParent():IsDonator() then return end
    ...
    if _G.autoPickup[self:GetParent():GetPlayerID()] ~= AUTOLOOT_OFF then
        ...
        for _,item in pairs(items_on_the_ground) do
            ...
            if containedItem then
                ...
                if not string.find(name, "item_socket_rune") and not string.find(name, "soul") and not string.find(name, "item_gold_bag") and not string.find(name, "asan") and not string.find(name, "item_piece") and not containedItem:IsNeutralDrop() then
