...
function modifier_generic_orb_effect_lua:OnAttack( params )
    ...
    if self:ShouldLaunch( params.target ) then
        ...
        if self.ability.OnOrbFire then 
            ...
            local essenceFlux = caster:FindAbilityByName("obsidian_essence_flux")

            if essenceFlux ~= nil and essenceFlux:GetLevel() > 0 then
                local essenceMod = caster:FindModifierByName("modifier_obsidian_essence_flux")
                if essenceMod ~= nil then
                    essenceMod:ProcEssenceFlux(caster, essenceFlux)
                end
            end
...
function modifier_generic_orb_effect_lua:FlagExist(a,b)--Bitwise Exist
    ...
    if 'number' ~= type(a) then return false end
    if 'number' ~= type(b) then return false end
